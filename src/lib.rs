...
impl NetBox {
    ...
    pub async fn get_choices(
        &mut self,
        path: &str,
    ) -> Result<HashMap<String, Vec<structs::Choice>>> {
        let url = self.base_url.clone();
    ...
    pub async fn ipam_ip_addresses(&mut self) -> Result<structs::Response<structs::IPAddress>> {
        let url = self.base_url.clone();
        let url = format!("{}/ipam/ip-addresses", url);
        let body = self.get(&url).await?;
        ...
        let count = v
            ...
            .as_i64()
            .unwrap_or_else(|| unimplemented!());
        let mut resp = structs::Response::new(count as usize);

        println!("{:?}", v);

        let err = Box::<dyn Error>::from("missing results".to_string());

        let v = v
            .get("results")
            .ok_or_else(|| Box::<dyn Error>::from("missing results".to_string()))?
            .as_array()
            .ok_or_else(|| Box::<dyn Error>::from("results not an array".to_string()))?;

        for v in v {
            resp.push(structs::IPAddress::parse(v)?);
        }
